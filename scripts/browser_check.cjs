...
(async () => {
  ...
  const total = Number(await page.locator('[data-publication-archive]').getAttribute('data-publication-total'));
  const pageCount = Math.min(total, 100);
  assert.equal(await visible.count(), pageCount);
  const waitForSearch = async (target = page) => target.waitForFunction(() => {
    const input = document.querySelector('[data-publication-search]');
    const params = new URL(location.href).searchParams;
    return (params.get('q') || params.get('author') || '') === input.value.trim() &&
      /^Showing|^0 publications$/.test(document.querySelector('[data-publication-count]').textContent);
  });
  const firstTitle = await visible.first().locator('h2').textContent();
  await page.locator('[data-publication-pagination]').first().getByText('Next', { exact: true }).click();
  await page.waitForURL('**/publications/page/2/');
  assert.equal(await visible.count(), Math.min(100, total - 100));
  assert.notEqual(await visible.first().locator('h2').textContent(), firstTitle);
  await page.goto(base + '/publications/');
  const search = page.locator('[data-publication-search]');
  await search.fill('Grier 2025'); await waitForSearch(); assert.equal(await visible.count(), 1);
  await search.fill('Woisetschlager'); await waitForSearch(); assert(await visible.count() > 0);
  await search.fill('Michael Dang’ana'); await waitForSearch(); assert(await visible.count() > 0);
  await search.fill('no-result-xyz'); await waitForSearch(); assert.equal(await visible.count(), 0);
  assert(await page.locator('[data-publication-empty]').isVisible());
  await page.locator('[data-publication-clear]').click(); await waitForSearch(); assert.equal(await visible.count(), pageCount);
  for (const button of await page.locator('[data-publication-tag]').all()) {
    await button.click();
    await page.waitForFunction(tag => new URL(location.href).searchParams.get('tag') === (tag === 'all' ? null : tag), await button.getAttribute('data-publication-tag'));
  ...
  await page.waitForFunction(() => new URL(location.href).searchParams.get('research') === 'quantum-computing-systems');
  const quantumCount = await visible.count(); assert(quantumCount > 0 && quantumCount < total);
  await page.reload(); await waitForSearch(); assert.equal(await visible.count(), quantumCount, 'Filters must survive reload');
  await page.goto(base + '/publications/?author=Grier+Jones'); await waitForSearch(); assert(await visible.count() >= 2);
  await page.goto(base + '/publications/?tag=invalid&research=invalid'); assert.equal(await visible.count(), pageCount);
  ...
  assert.equal(await plain.locator('[data-publication-card]:visible').count(), pageCount);
  assert.equal(await plain.locator('[data-publication-controls]').isVisible(), false);
  await plain.locator('[data-publication-pagination]').first().getByText('Next', { exact: true }).click();
  await plain.waitForURL('**/publications/page/2/');
  assert.equal(await plain.locator('[data-publication-card]:visible').count(), Math.min(100, total - 100));
  await noJS.close();
  ...
  await waitForSearch(blocked);
  assert.equal(await blocked.locator('[data-publication-card]:visible').count(), 1);
  await blockedStorage.close();

  // Filtering must search beyond the first page while keeping the DOM bounded.
  const largeContext = await createContext({ viewport: { width: 390, height: 844 } });
  const largePage = await largeContext.newPage();
  const seed = JSON.parse(fs.readFileSync(path.join(root, 'publications/index.json'), 'utf8'))[0];
  const largeIndex = Array.from({ length: 10000 }, (_, i) => ({ ...seed,
    title: `Stress publication ${i}`, url: seed.url, search: `${seed.search} stress record${i}` }));
  let indexLoads = 0;
  await largeContext.route('**/publications/index.json', route => {
    indexLoads++;
    return route.fulfill({ json: largeIndex });
  });
  await largePage.goto(base + '/publications/');
  assert.equal(indexLoads, 0, 'Ordinary browsing should not load the search index');
  await largePage.locator('[data-publication-search]').fill('stress');
  await waitForSearch(largePage);
  assert.equal(await largePage.locator('[data-publication-card]').count(), 100);
  assert.match(await largePage.locator('[data-publication-count]').textContent(), /of 10000 publications/);
  await largePage.locator('[data-publication-pagination]').first().getByText('Next', { exact: true }).click();
  await largePage.waitForFunction(() => document.querySelector('[data-publication-count]').textContent.startsWith('Showing 101'));
  assert.equal(await largePage.locator('[data-publication-card]').count(), 100);
  await largePage.goBack();
  await largePage.waitForFunction(() => document.querySelector('[data-publication-count]').textContent.startsWith('Showing 1–100'));
  const searchStarted = performance.now();
  await largePage.locator('[data-publication-search]').fill('record9999');
  await waitForSearch(largePage);
  report.publicationSearch10000Ms = Math.round(performance.now() - searchStarted);
  assert.equal(await largePage.locator('[data-publication-card]').count(), 1);
  assert.equal(await largePage.locator('[data-publication-card] h2').textContent(), 'Stress publication 9999');
  assert.equal(indexLoads, 1, 'Reuse the index between searches and pages');
  await largeContext.close();

  const retryContext = await createContext();
  const retryPage = await retryContext.newPage();
  await retryContext.route('**/publications/index.json', route => route.abort());
  await retryPage.goto(base + '/publications/');
  await retryPage.locator('[data-publication-search]').fill('Q-DICE');
  await retryPage.locator('[data-publication-error]').waitFor({ state: 'visible' });
  assert.equal(await retryPage.locator('[data-publication-card]').count(), pageCount);
  assert(await retryPage.locator('[data-publication-pagination] a[rel="next"]').first().isVisible());
  await retryContext.unroute('**/publications/index.json');
  await retryPage.locator('[data-publication-retry]').click();
  await waitForSearch(retryPage);
  assert.equal(await retryPage.locator('[data-publication-card]').count(), 1);
  assert.equal(await retryPage.locator('[data-publication-error]').isVisible(), false);
